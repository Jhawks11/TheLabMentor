...
Krebs Cycle: Succinic 0-9.3, Fumaric 0-0.94, Malic 0.06-1.8, 2-Oxoglutaric 0-35, Aconitic 6.8-28, Citric 0-507.
Neurotransmitter Metabolites: HVA 0.8-3.6, VMA 0.46-3.7, HVA/VMA Ratio 0.16-1.8, DOPAC 0.08-3.5, 5-HIAA 0-4.3, Quinolinic 0.85-3.9, Kynurenic 0-2.2.
Nutritional: MMA-B12 0-2.3, Pyridoxic-B6 0-34, Pantothenic-B5 0-10, Glutaric-B2 0.04-0.36, Ascorbic-C 10-200, CoQ10 0.17-39, Methylcitric-Biotin 0.19-2.7.
Detoxification: Pyroglutamic-Glutathione 10-33, 2-Hydroxybutyric 0.03-1.8, Orotic-Ammonia 0.06-0.54.
Folate: Uracil 0-9.7, Thymine 0-0.56.
Fatty Acid Oxidation: Ethylmalonic 0.44-2.8, Adipic 0.04-3.8, 3-Hydroxybutyric 0-3.1, Suberic 0.18-2.2.`
};
...
exports.handler = async (event) => {
  ...
  try {
    const { sessionId, panel, fileData, fileType } = JSON.parse(event.body);
    ...
    const systemPrompt = `You are an expert functional medicine practitioner for Jennifer Hawks Health. Interpret ${panelLabel} results using ONLY these JHH functional ranges:

${ranges}

Flag markers outside JHH ranges as "flagged-high" or "flagged-low". Borderline = within 10% of the edge. Write in JHH's warm, empowering, science-meets-soul voice.

Respond ONLY with valid JSON — no preamble, no markdown:
{
  "panelType": "${panelLabel}",
  "functionalFlags": [
    {"marker": "name", "value": "patient value + units", "functionalRange": "JHH range", "status": "optimal|borderline|flagged-high|flagged-low", "note": "1 warm sentence"}
  ],
  "summary": "3-4 warm empathetic sentences about the overall pattern",
  "rootCauses": ["pattern 1", "pattern 2"],
  "nextSteps": ["step 1", "step 2"]
}
Include ALL markers. Give 3-6 root causes and 5-8 next steps. No text outside JSON.`;

    const isImage = fileType && fileType.startsWith('image/');
    const content = [
      {
        type: isImage ? 'image' : 'document',
        source: { type: 'base64', media_type: isImage ? fileType : 'application/pdf', data: fileData }
      },
      { type: 'text', text: `Analyze this ${panelLabel} using JHH ranges and return JSON.` }
    ];
